...
const generateAccessTokenAndRefreshToken = async (userId) => {
	try {
		...
		throw new ApiError(
			500,
			"failed to generate access Token and Referesh Token.",
		);
	}
};
...
const registerUser = asyncHandler(async (req, res) => {
	...
	if (coverImageLocalPath !== "") {
		globalThis.coverImage = await uploadOnCloudinary(coverImageLocalPath);
	}
...
const logOutUser = asyncHandler(async (req, res) => {
	if (req.body.email !== req.user.email) {
		throw new ApiError(400, "Wrong email id");
	}
		...
		return res
			...
			.json(
				new ApiResponse(
					...
					"Tokens are refreshed successfuly. ",
				),
			);
	} catch (error) {
...
const changedCurrentPassword = asyncHandler(async (req, res) => {
	...
	if (!(newPassword === conformPassword)) {
		throw new ApiError(
			400,
			"new password and comform password are must be  same. ",
		);
	}
	...
	if (!isPasswordCorrect) {
		throw new ApiError(400, "password not match. please try again. ");
	}
...
const getCurrentUser = asyncHandler(async (req, res) => {
	return res
		.status(200)
		.json(new ApiResponse(200, req.user, "current user fatched successfuly. "));
});

const updateUserAccoundDetails = asyncHandler(async (req, res) => {
	...
	return res
		.status(200)
		.json(
			new ApiResponse(200, user, "user account detials update successful. "),
		);
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
	...
	if (!avatar?.url) {
		throw new ApiError(500, "Error while uploading cloudnary.");
	}
	...
	const splitedOldUrl = oldAvatarUrl.split(/[/.]/);
	const oldAvatarFileName = splitedOldUrl[splitedOldUrl.length - 2];
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
	...
	const splitedOldUrl = oldCoverImageUrl.split(/[/.]/);
	const oldCoverImageFileName = splitedOldUrl[splitedOldUrl.length - 2];
	cloudinary.uploader.destroy(oldCoverImageFileName);
	return res
		.status(200)
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
	...
	if (!channel.length) {
		throw new ApiError(400, "channel is does not exist.");
	}
